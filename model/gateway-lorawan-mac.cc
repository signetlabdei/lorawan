...
GatewayLorawanMac::Send(Ptr<Packet> packet)
{
    ...
    if (m_channelHelper->GetWaitingTime(CreateObject<LogicalLoraChannel>(frequencyMHz)) > Time(0))
    {
        // We cannot send now!
    ...
    double sendingPower =
        m_channelHelper->GetTxPowerForChannel(CreateObject<LogicalLoraChannel>(frequencyMHz));

    // Add the event to the channelHelper to keep track of duty cycle
    m_channelHelper->AddEvent(duration, CreateObject<LogicalLoraChannel>(frequencyMHz));
...
GatewayLorawanMac::GetWaitingTime(double frequencyMHz)
{
    ...
    return m_channelHelper->GetWaitingTime(CreateObject<LogicalLoraChannel>(frequencyMHz));
}
} // namespace lorawan
} // namespace ns3
