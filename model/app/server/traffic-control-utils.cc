...
double
TrafficControlUtils::OptimizeDutyCycleMaxMin(const devices_t& devs,
                                             const double limit,
                                             output_t& output)
{
    ...
    for (int i = 0; i < dm.bound; ++i)
        solver->MakeBoolVarArray(nsettings,
                                 "Which duty-cycle value is assigned to device " +
                                     std::to_string(i),
                                 &(x[i]));
    ...
    for (int i = 0; i < dm.bound; ++i)
    {
        MPConstraint* constraint =
            solver->MakeRowConstraint(0.0,
                                      infinity,
                                      "Linearize objective for device " + std::to_string(i));
...
void
TrafficControlUtils::OptimizeDutyCycleMax(const devices_t& devs,
                                          const double limit,
                                          output_t& output)
{
    NS_LOG_FUNCTION("Num devs: " + std::to_string(devs.size())
...
const std::vector<double> TrafficControlUtils::m_dutycycles =
    std::vector<double>{1.0 / std::pow(2.0, 7.0),
                        1.0 / std::pow(2.0, 8.0),
                        1.0 / std::pow(2.0, 9.0),
                        1.0 / std::pow(2.0, 10.0),
                        1.0 / std::pow(2.0, 11.0),
                        1.0 / std::pow(2.0, 12.0),
                        1.0 / std::pow(2.0, 13.0),
                        1.0 / std::pow(2.0, 14.0),
                        1.0 / std::pow(2.0, 15.0),
                        0.0};
