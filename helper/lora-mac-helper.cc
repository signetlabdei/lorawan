  ...
  if (m_deviceType == ED)
    {
      ...
      switch (m_region)
        {
        case LoraMacHelper::EU868:
          {
            ConfigureForEu868Region (edMac);
            break;
          }
          case LoraMacHelper::US915:
          {
            ConfigureForUS915Region (edMac);
            break;
          }
        default:
          {
...
void
LoraMacHelper::ConfigureForEu868Region (Ptr<EndDeviceLoraMac> edMac) const
{
...
void
LoraMacHelper::ConfigureForEu868Region (Ptr<GatewayLoraMac> gwMac) const
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
LoraMacHelper::ApplyCommonEu868Configurations (Ptr<LoraMac> loraMac) const
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
LoraMacHelper::ConfigureForUS915Region (Ptr<EndDeviceLoraMac> edMac) const
{
  NS_LOG_FUNCTION_NOARGS ();

  ApplyCommonUS915Configurations (edMac);
  ...
  // Matrix to know which DataRate the GW will respond with //
  ////////////////////////////////////////////////////////////
  LoraMac::ReplyDataRateMatrix matrix = {{{{10,9,8,8}},
                                          {{11,10,9,8}},
                                          {{12,11,10,9}},
                                          {{13,12,11,10}},
                                          {{13,13,12,11}}}};
...
void
LoraMacHelper::ConfigureForUS915Region (Ptr<GatewayLoraMac> gwMac) const
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  ApplyCommonUS915Configurations (gwMac);
...
void
LoraMacHelper::ApplyCommonUS915Configurations (Ptr<LoraMac> loraMac) const
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  //////////////
  LogicalLoraChannelHelper channelHelper;
  channelHelper.AddSubBand (902, 928, 1, 30);

  //////////////////////
  // Default channels //
  //////////////////////

  // for (double ch0_63 = 902.3; ch0_63<=914.9+0.2; ch0_63+=0.2){
  //   Ptr<LogicalLoraChannel> lc0_63 = CreateObject<LogicalLoraChannel> (ch0_63, 0, 3);
  //   channelHelper.AddChannel (lc0_63);
  // }
  for (double ch64_71 = 903.0; ch64_71<=914.2+1.6; ch64_71+=1.6){
    Ptr<LogicalLoraChannel> lc64_71 = CreateObject<LogicalLoraChannel> (ch64_71, 4, 4);
    channelHelper.AddChannel (lc64_71);
  }
