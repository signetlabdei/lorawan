/*
 * Copyright (c) 2022 Alessandro Aimi
 *
...
LoratapHeader::Print(std::ostream& os) const
{
    ...
    os << "(lt_version " << unsigned(m_lt_version) << " lt_padding " << unsigned(m_lt_padding)
       << " lt_length " << unsigned(m_lt_length) << ") frequency " << unsigned(m_frequency)
       << " bandwidth " << unsigned(m_bandwidth) << " sf " << unsigned(m_sf) << " packet_rssi "
       << unsigned(m_packet_rssi) << " max_rssi " << unsigned(m_max_rssi) << " current_rssi "
       << unsigned(m_current_rssi) << " snr " << unsigned(m_snr) << " sync_word "
       << unsigned(m_sync_word);
}
