  ...
  static TypeId tid =
      TypeId ("ns3::UdpForwarder")
          ...
          .AddAttribute ("RemotePort", "The destination port of the outbound packets",
                         UintegerValue (1700), MakeUintegerAccessor (&UdpForwarder::m_peerPort),
...
UdpForwarder::StartApplication (void)
{
  ...
  m_statsEvent = Simulator::Schedule (MilliSeconds (1000 * stat_interval),
                                      &UdpForwarder::CollectStatistics, this);
}
...
UdpForwarder::ThreadUp (void)
{
  ...
  *(uint32_t *) (buff_up + 8) = net_mac_l;
  ...
  if (send_report == true)
    {
      report_ready = false;
      j = snprintf ((char *) (buff_up + buff_index), TX_BUFF_SIZE - buff_index, "%s",
                    status_report);
      if (j > 0)
        {
          buff_index += j;
  ...
  if ((int) difftimespec (m_downRecvTime, m_downSendTime) < keepalive_time)
    {
      ...
      m_downEvent = Simulator::Schedule (MicroSeconds (pull_timeout.tv_usec),
                                         &UdpForwarder::SockDownTimeout, this);
    }
  else
    {
          ...
          if (m_reqAck)
            ...
            { /* if that packet was not already acknowledged */
              ...
              NS_LOG_INFO ("[down] PULL_ACK received in "
                           << (int) (1000 * difftimespec (m_downRecvTime, m_downSendTime))
                           << " ms");
            }
        }
      else
  ...
  if ((txpkt.freq_hz < tx_freq_min[txpkt.rf_chain]) ||
      (txpkt.freq_hz > tx_freq_max[txpkt.rf_chain]))
    {
      ...
      NS_LOG_ERROR ("Packet REJECTED, unsupported frequency - "
                    << (unsigned) txpkt.freq_hz
                    << " (min:" << (unsigned) tx_freq_min[txpkt.rf_chain]
...
UdpForwarder::CollectStatistics (void)
{
  ...
  snprintf (buf, 120, "# CRC_OK: %.2f%%, CRC_FAIL: %.2f%%, NO_CRC: %.2f%%\n", 100.0 * rx_ok_ratio,
            100.0 * rx_bad_ratio, 100.0 * rx_nocrc_ratio);
  ss << buf;
  snprintf (buf, 120, "# RF packets forwarded: %u (%u bytes)\n", meas_up_pkt_fwd,
            meas_up_payload_byte);
  ss << buf;
  snprintf (buf, 120, "# PUSH_DATA datagrams sent: %u (%u bytes)\n", meas_up_dgram_sent,
  ...
  ss << buf;
  snprintf (buf, 120, "# PULL_DATA sent: %u (%.2f%% acknowledged)\n", meas_dw_pull_sent,
            100.0 * dw_ack_ratio);
  ss << buf;
  snprintf (buf, 120, "# PULL_RESP(onse) datagrams received: %u (%u bytes)\n", meas_dw_dgram_rcv,
            meas_dw_network_byte);
  ss << buf;
  snprintf (buf, 120, "# RF packets sent to concentrator: %u (%u bytes)\n",
            (meas_nb_tx_ok + meas_nb_tx_fail), meas_dw_payload_byte);
  ss << buf;
  snprintf (buf, 120, "# TX errors: %u\n", meas_nb_tx_fail);
  ss << buf;
  if (meas_nb_tx_requested != 0)
    {
      snprintf (buf, 120, "# TX rejected (collision packet): %.2f%% (req:%u, rej:%u)\n",
                100.0 * meas_nb_tx_rejected_collision_packet / meas_nb_tx_requested,
                meas_nb_tx_requested, meas_nb_tx_rejected_collision_packet);
      ss << buf;
      snprintf (buf, 120, "# TX rejected (collision beacon): %.2f%% (req:%u, rej:%u)\n",
                100.0 * meas_nb_tx_rejected_collision_beacon / meas_nb_tx_requested,
                meas_nb_tx_requested, meas_nb_tx_rejected_collision_beacon);
      ss << buf;
      snprintf (buf, 120, "# TX rejected (too late): %.2f%% (req:%u, rej:%u)\n",
                100.0 * meas_nb_tx_rejected_too_late / meas_nb_tx_requested, meas_nb_tx_requested,
                meas_nb_tx_rejected_too_late);
      ss << buf;
      snprintf (buf, 120, "# TX rejected (too early): %.2f%% (req:%u, rej:%u)\n",
                100.0 * meas_nb_tx_rejected_too_early / meas_nb_tx_requested, meas_nb_tx_requested,
  ...
  if (gps_fake_enable == true)
    {
      snprintf (status_report, STATUS_SIZE,
                ...
                stat_timestamp, reference_coord.lat, reference_coord.lon, reference_coord.alt,
                meas_nb_rx_rcv, meas_nb_rx_ok, meas_up_pkt_fwd, 100.0 * up_ack_ratio,
                meas_dw_dgram_rcv, meas_nb_tx_ok);
    }
  else
    {
      snprintf (status_report, STATUS_SIZE,
                ...
                "\"dwnb\":%u,\"txnb\":%u}",
                stat_timestamp, meas_nb_rx_rcv, meas_nb_rx_ok, meas_up_pkt_fwd,
                100.0 * up_ack_ratio, meas_dw_dgram_rcv, meas_nb_tx_ok);
    }
  report_ready = true;
